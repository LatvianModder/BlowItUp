...
import com.latmod.lib.math.Pos2D;
...
public class WorldRenderer
{
    ...
    public void render(IWindow window)
    {
        ...
        if(world.level.ambient_light < 1F && isLightDirty)
        {
            ...
            if(world.clientPlayer.getFlag(Entity.LIGHT))
            {
    ...
    {
        if(dist <= 0F || x < 0 || y < 0 || x >= world.level.width || y >= world.level.height)
        {
            return;
        }
        ...
        if(lightMap[x][y].c < dist)
        {
            if(blocked)
            {
                ...
                if(!t.getFlag(Tile.FLAG_IS_TRANSPARENT))
                {
                    return;
                }
            }

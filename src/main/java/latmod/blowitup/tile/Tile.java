...
public class Tile extends FinalIDObject
{
    ...
    public static Tile readFromIO(DataInput io) throws Exception
    {
        ...
        if(tile.getFlag(FLAG_HAS_TEXTURE_NAME))
        {
            tile.texture_name = io.readUTF();
        }
        if(tile.getFlag(FLAG_IS_LIT))
        {
            tile.light_value = io.readFloat();
        }
        return tile;
    }
    ...
    public void writeToIO(DataOutput io) throws Exception
    {
        ...
        io.writeByte(byteID);
        io.writeByte(flags);
        if(getFlag(FLAG_HAS_TEXTURE_NAME))
        {
            io.writeUTF(texture_name);
        }
        if(getFlag(FLAG_IS_LIT))
        {
            io.writeFloat(light_value);
        }
    }
    ...
    {
        if(o.has("texture_name"))
        {
            texture_name = o.get("texture_name").getAsString();
        }
        if(o.has("light_value"))
        {
            light_value = o.get("light_value").getAsFloat();
        }
        if(light_value > 0F)
        {
            setFlag(Tile.FLAG_IS_TRANSPARENT, true);
        }

        if(o.has("transparent"))
        {
            setFlag(Tile.FLAG_IS_TRANSPARENT, o.get("transparent").getAsBoolean());
        }
        if(o.has("can_collide"))
        {
            setFlag(Tile.FLAG_CAN_COLLIDE, o.get("can_collide").getAsBoolean());
        }
        if(o.has("can_explode"))
        {
            setFlag(Tile.FLAG_CAN_EXPLODE, o.get("can_explode").getAsBoolean());
        }
    }
}
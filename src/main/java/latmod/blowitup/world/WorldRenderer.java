...
public class WorldRenderer
{
	...
	public void render(GameRenderer r)
	{
		...
		{
			floorTexture = r.textureManager.getTexture(Resource.getTexture("world/floor.png"));
		}
		...
		double screenW = Main.inst.width / s;
		double screenH = Main.inst.height / s;
		...
		if(isDirty)
		{
			...
			{
				Texture t = e.getValue().getTexture(r);
				if(t != null)
				{
					t.bind();
					Pos2I p = e.getKey();
		...
		for(Entity e : world.entities)
			e.onRender(r);
		world.clientPlayer.onRender(r);

		GLHelper.texture.disable();
		if(world.level.ambient_light < 1F)
			Renderer.renderList(lightListID);
	...
	private void putLight(int x, int y, float dist, boolean blocked)
	{
		...
		if(lightMap[x][y].c < dist)
		{
			if(blocked)
			{
				...
				if(!t.flags[Tile.FLAG_IS_TRANSPARENT]) return;
			}
